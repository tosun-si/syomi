...
case class Validator[+A](private val element: A, errorMessages: List[String]) {
  ...
  def getOrElseThrow[E <: RuntimeException](exceptionClass: Class[E]): A = {
    errorMessages match {
      case err: List[String] if errorMessages.isEmpty => element
  ...
  def toEither: Either[List[String], A] = {
    errorMessages match {
      case err: List[String] if errorMessages.isEmpty => Right(element)
  ...
  def toErrorsOption: Option[List[String]] = {
    errorMessages match {
      case err: List[String] if err.isEmpty => None
